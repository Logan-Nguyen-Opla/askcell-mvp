...
class CellEngine:
    ...
    def get_gene_expression(self, gene_name: str) -> dict:
        ...
        # Case-insensitive resolution: scRNA-seq symbols are usually upper-case
        # (CD3D, PDCD1) but users may type cd3d.
        if gene not in var_names:
            lower_map = {g.lower(): g for g in var_names}
            resolved = lower_map.get(gene.lower())
            if resolved is None:
                return {
                    "gene": gene_name,
                    "error": (
                        f"Gene '{gene_name}' was not found in the loaded "
                        f"dataset's {len(var_names)} measured features."
                    ),
                }
            gene = resolved

        gene_idx = var_names.get_loc(gene)
        column = self.adata.X[:, gene_idx]

        # adata.X is frequently a sparse CSR/CSC matrix. Densify the single
        # column (cheap) and flatten to a 1-D NumPy array.
        if sparse.issparse(column):
            expr = column.toarray().ravel()
        else:
            expr = np.asarray(column).ravel()
